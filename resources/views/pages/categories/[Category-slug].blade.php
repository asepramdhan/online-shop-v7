...
state(['category']);
?>
<x-app-layout title="Products in {{ $category->name }}">
  @volt
  <div>
    <h1 class="mb-5">Products by Category : {{ $category->name }}</h1>
    <div class="row">
      @foreach ( $category->products->load(['user', 'category']) as $product )
